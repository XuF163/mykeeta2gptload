...
class _SafeTextStream:
    """Wrap a text stream so console logging won't crash on UnicodeEncodeError.

    Windows consoles often use a legacy encoding (e.g. gbk/cp936) which cannot
    represent emoji and some symbols. We keep the original unicode for file logs
    (utf-8 handler) while replacing unencodable chars for the console output.
    """
...
class Logger:
    ...
    def _setup_logger(self):
        ...
        # 控制台处理器
        console_handler = logging.StreamHandler(_SafeTextStream(sys.stdout))
